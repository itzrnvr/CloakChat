...
def test_benign_text_returns_empty(provider, model, api_key, base_url, system_prompt):
...
def test_person_name_triggers_ambiguity_not_replacement(provider, model, api_key, base_url, system_prompt):
    """PERSON names go to ambiguities (or replacements for local models)."""
    result, _ = detect(
    ...
    # Person names may appear in ambiguities (Google GenAI) or replacements (local models)
    ...
    total_person = len(person_ambiguities) + len(person_replacements)
    assert total_person > 0, "No PERSON entities found for 'amitabh weds mandy'."
    for a in person_ambiguities:
        assert a.original and a.entity_type == "PERSON" and a.suggested_replacement and a.reason

def test_email_auto_replaced(provider, model, api_key, base_url, system_prompt):
    result, _ = detect(
...
def test_phone_number_auto_replaced(provider, model, api_key, base_url, system_prompt):
    result, _ = detect(
...
def test_mixed_pii_detection(provider, model, api_key, base_url, system_prompt):
    result, _ = detect(
...
def test_detection_result_fields_are_well_formed(provider, model, api_key, base_url, system_prompt):
    result, _ = detect(
...
def test_person_names_never_in_replacements(provider, model, api_key, base_url, system_prompt):
    """PERSON names go to ambiguities with Google GenAI, but local models may replace them."""
    result, _ = detect(
    ...
    person_replacements = [r for r in result.replacements if r.entity_type == "PERSON"]
    person_ambiguities = [a for a in result.ambiguities if a.entity_type == "PERSON"]
    # Local models may put person names in replacements instead of ambiguities
    total = len(person_replacements) + len(person_ambiguities)
    assert total > 0, "No PERSON entities detected in 'Raj met Priya at the cafe'."

def test_playbook_keep_rule_respected(provider, model, api_key, base_url, system_prompt):
    result, _ = detect(
...
def test_playbook_anonymize_rule_respected(provider, model, api_key, base_url, system_prompt):
    result, _ = detect(
